  ...
  async (
    {
      ...
      userId,
      userEmail,
    }: { titleList: string; userId: string; userEmail: string },
    thunkAPI,
  ) => {
  ...
  async (
    {
      ...
      listId,
      todoId,
    }: { userId: string; listId: string; todoId: number },
    thunkAPI,
  ) => {
    try {
      const listRef = doc(db, "users", userId, "todoLists", listId);
      ...
      const updatedTodos = (listData.todos || []).filter(
...
export const todoListsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        toggleTodoCompletedInFirestore.fulfilled,
        (
          state,
